...
def restore_agent(agent, restore_path, restore_epoch):
    ...
    saved_state = load_dict.get('agent', load_dict)

    # Current agent state (structure) as a dict.
    target_state = flax.serialization.to_state_dict(agent)

    def _merge_states(saved, target, path=''):
        """Recursively merge saved -> target when keys and shapes match.

        Returns a new dict with values taken from saved when possible, otherwise
        keeping target's values. Logs mismatches for debugging.
        """
        if not isinstance(target, dict):
            # Leaf node: check saved is compatible (same shape/type)
            return saved if (not isinstance(saved, dict)) else target
        ...
        for k, v in target.items():
            ...
            if k in saved:
                ...
                if isinstance(v, dict) and isinstance(s, dict):
                    out[k] = _merge_states(s, v, path=full_key)
                else:
                    # leaf - try to copy if shapes/types compatible
                    try:
                        # convert to arrays to check shape when possible
                        sv = jnp.asarray(s)
                        tv = jnp.asarray(v)
                        if sv.shape == tv.shape:
                            ...
                            out[k] = v
                            print(f"restore_agent: shape mismatch at {full_key}: saved {sv.shape} vs target {tv.shape}; keeping target")
                    except Exception:
                        # if we can't compare shapes, keep saved value to be permissive
                        out[k] = s
            else:
                out[k] = v
                print(f"restore_agent: key missing in checkpoint at {full_key}; keeping target")
        return out

    merged_state = _merge_states(saved_state, target_state)

    # Use from_state_dict to recreate agent from merged state
    agent = flax.serialization.from_state_dict(agent, {'agent': merged_state}['agent'])

    print(f'Restored from {restore_path} (merged parameters; missing/mismatched keys preserved)')
